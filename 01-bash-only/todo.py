"""s03: TodoWrite MVP with local Qwen3.5-2B.
...
SYSTEM = (
    ...
    "item should be `in_progress` at a time. Reply with a final message only "
...
def agent_loop(query: str):
    ...
    for turn in range(1, MAX_ROUNDS + 1):
        ...
        print(f"→ [3] rounds_since_todo = {rounds_since_todo}"
    ...
    _box("⛔ MAX ROUNDS", f"达到 {MAX_ROUNDS} 轮上限, 强制退出", char="═")


if __name__ == "__main__":
    q = " ".join(sys.argv[1:]) or "Create mypkg/ with __init__.py and utils.py (utils has an add(a,b) function). Plan with the todo tool first."
    agent_loop(q)
