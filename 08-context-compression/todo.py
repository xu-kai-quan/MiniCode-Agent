"""MiniCode Agent v8 — context compression on top of v7 session persistence.

继承 v7 的 Session 持久化 + v6 的 spawn_agent + v5 的双后端流式. v8 新增:

  自动 + 手动 context 压缩 — 当本 turn 的真实 prompt_tokens 超过 CONTEXT_LIMIT
  的 COMPRESS_AT 比例 (默认 70%) 时, 下一 turn 开始前先压一次 history:

    保头: history[0] (system prompt)
    摘要: history[1:split] 让 LLM 总结成一条 <compressed>...</compressed> user 消息
          tool_results 不保留全文 (Read 全文 / Grep 输出 / bash stdout 太占 token)
    保尾: history[split:] 最近 COMPRESS_KEEP_TAIL 条 (默认 10), 含完整 tool_call/result

  切分点必须落在 tool_call/tool_result 配对外侧 — OpenAI API 不允许拆对.
  LLM 总结失败时降级到机械裁剪 (从 tool_calls + path 字段抠骨架信息).
...
# ----- Session 持久化 (v7 新增) -----
# 跨进程保存的 session 存在用户 home 下 — 跨 cwd 通用, 跟 Claude Code 的设计类似.
# 用户 cd 到任何项目目录都能 /save / /load. 单文件 JSON 一个 session.
SESSIONS_DIR = Path(
...
# ----- Context 压缩 (v8 新增) -----
# 模型 context 上限, 跟 backend 强相关. 用户可显式设 MINICODE_CONTEXT_LIMIT 覆盖.
# 默认值: MiniMax-M2.7 ~ 200K, Ollama 7B 默认 32K (qwen2.5-coder-7b-32k).
# 这是 *prompt token* 上限, 不含 completion. 我们关心的也是 prompt 那一边.
_CONTEXT_LIMIT_DEFAULTS = {"minimax": 200_000, "ollama": 32_000}
CONTEXT_LIMIT = int(os.environ.get(
    "MINICODE_CONTEXT_LIMIT",
    str(_CONTEXT_LIMIT_DEFAULTS.get(BACKEND, 32_000)),
))
# 触发压缩的比例 — 当本 turn 真实 prompt_tokens 超过 CONTEXT_LIMIT * 此比例,
# 下一 turn 开始前先压一次. 0.7 留 30% 给压缩后再跑几轮.
COMPRESS_AT = float(os.environ.get("MINICODE_COMPRESS_AT", "0.7"))
# 压缩时保留尾部最后多少条 message (含 tool_call/tool_result 配对). 太小模型丢上下文,
# 太大压缩没省多少. 10 条 ≈ 最近 2-3 个完整工具调用循环.
COMPRESS_KEEP_TAIL = int(os.environ.get("MINICODE_COMPRESS_KEEP_TAIL", "10"))
# 压缩后摘要内容的最大字符. LLM 总结被指示控制在这个长度内.
COMPRESS_SUMMARY_MAX_CHARS = int(os.environ.get("MINICODE_COMPRESS_SUMMARY_MAX", "2000"))
...
class Message:
    ...
    def from_dict(cls, d: dict) -> "Message":
        """反序列化 — Session.from_dict 用. content 缺失时按空串处理 (assistant
        消息流式期被 Ctrl-C 截断的边角)."""
        return cls(
...
class Session:
    ...
    def to_dict(self) -> dict:
        """序列化为 JSON 友好的 dict. 用于 /save 命令.

        故意不存:
        - read_cache: 文件 mtime/size 在 load 时大概率已变, 强制重读更安全
        - is_subagent: 子 agent 不存 (主 agent 没法 spawn 一个保存的子 agent)
        - rounds_since_todo / codeblock_nag_count / estimated_turns / exact_turns:
          这些是当前轮的瞬时计数, 跨 session 没意义
        - todo: 含在 history 里 (todo 工具调用的结果在历史 message 里)... 但 TodoManager
          自己的 in-memory items 是模型最近一次 todo 调用的"权威列表", 不存就丢了
          → 还是存. (TodoManager 已经是 list[dict])

        存的字段 = 跨进程能恢复 "上次到哪了" 的最小集合.
        """
        return {
            "version": 1,  # schema 版本, 未来字段调整时 from_dict 能识别老格式
            "history": [m.to_dict() for m in self.history],
            "todo_items": list(self.todo.items),
    ...
    def from_dict(cls, d: dict) -> "Session":
        """反序列化. read_cache 永远是新空的 — load 后模型必须重新 Read 才能写."""
        version = d.get("version", 1)
        if version != 1:
            raise RuntimeError(f"unknown session schema version: {version}")
        sess = cls(
            history=[Message.from_dict(md) for md in d.get("history", [])],
            prompt_tokens_total=int(d.get("prompt_tokens_total", 0)),
            completion_tokens_total=int(d.get("completion_tokens_total", 0)),
        )
        sess.todo.items = list(d.get("todo_items", []))
        return sess
...
# =================================================================
#
# 痛点: history 永不裁剪, tool_results (Read 全文 / Grep 结果 / bash stdout) 累积
# 撑爆 context. 长任务跑十几个 turn 后 prompt token 撞上限.
#
# 策略: 保头 [system] + 摘要中段 (LLM 总结成一条 user 消息) + 保尾最近 N 条.
# 切分点必须在 tool_call/tool_result 配对边界外 — OpenAI API 不允许拆对.
# LLM 总结失败时降级到机械裁剪 (砍掉中段 tool_result content, 保 tool_call 骨架).

_COMPRESSION_MARKER = "<compressed>"
_COMPRESSION_MARKER_END = "</compressed>"

_COMPRESSION_PROMPT = textwrap.dedent("""\
    The conversation has grown too long for the context window. Summarize the
    messages between the system prompt and the most recent {tail_n} messages
    so the agent can continue without losing track of what has happened.

    Produce a single concise summary covering, in this order:
      1. The user's original task / current goal.
      2. Key facts the agent discovered (file paths read, important line
         numbers, error messages, test outcomes — only what's still relevant).
      3. Files that have been modified and what was changed.
      4. Outstanding todo items still in_progress / pending.
      5. The next concrete step the agent was about to take.

    Drop verbatim tool output (Read content, Grep matches, bash stdout). The
    agent can re-read files if it needs them. DO NOT invent facts that aren't
    in the messages below.

    Stay under {max_chars} characters. No code blocks, no markdown headings —
    plain prose with short paragraph breaks is fine.
...
def _msg_repr_for_summary(m: Message, max_tool_chars: int = 400) -> str:
    """把一条 Message 渲染成给摘要 LLM 看的紧凑形式. tool_results 截断."""
    role = m.role
    if role == "system":
        return ""  # system 永远在 [0], 不进摘要
    if role == "user":
    ...
    if role == "assistant":
        ...
        return f"[assistant] {' '.join(parts) if parts else '(no content)'}"
    if role == "tool":
        body = m.content or ""
        if len(body) > max_tool_chars:
            body = body[:max_tool_chars] + f"...(+{len(body) - max_tool_chars} chars)"
        return f"[tool_result] {body}"
    return f"[{role}] {m.content or ''}"


def _find_compression_split(history: list[Message], keep_tail: int) -> int | None:
    """决定从哪个 index 开始保留 (尾部). 返回 split: history[1:split] 进摘要,
    history[split:] 原样保留 (system [0] 永远保).

    约束: split 必须正好落在一对 (assistant with tool_calls, tool_result*) 之外
    — 不能拆开. 如果初始 split 落在配对中间, 往前 (向更早的方向) 推一步.

    返回 None 表示 history 太短不值得压.
    """
    n = len(history)
    if n <= keep_tail + 2:
        return None  # 总共没多少, 压缩没收益
    # 初始 split: 保留尾部 keep_tail 条
    split = n - keep_tail
    if split <= 1:
        return None  # 摘要段为空
    # 修正: 如果 history[split] 是 tool 消息, 说明它的 assistant tool_call 在前面,
    # 切分点会拆配对 — 把 split 往前推到那个 assistant 之前 (即让整对都进尾部).
    # 同样地, 如果 history[split-1] 是带 tool_calls 的 assistant 但 split 把它的
    # tool_results 留在尾部却把它本身丢进摘要, 也要往前推.
    while split > 1:
        if history[split].role == "tool":
            # tool_result 不能落在 split 起点, 否则它的 tool_call 在摘要侧, 配对断
            split -= 1
            continue
        prev = history[split - 1]
        if prev.role == "assistant" and prev.tool_calls:
            # 上一条是带 tool_calls 的 assistant — 它的 tool_results 紧随其后,
            # 它们都得跟着 split 一起进尾部. 把 split 推到这个 assistant 之前.
            split -= 1
            continue
        break
    if split <= 1:
...
def _build_summary_body(history: list[Message], split: int) -> str:
    """渲染 history[1:split] 给摘要 LLM. system 跳过."""
    lines = []
    for m in history[1:split]:
        rendered = _msg_repr_for_summary(m)
        if rendered:
            lines.append(rendered)
    return "\n".join(lines)


def _summarize_via_llm(llm: "LLMClient", body: str, tail_n: int) -> str | None:
    """调 LLM 总结. 失败返回 None — 调用方走机械兜底.

    走流式路径 (chat_stream + 收集 content), 跟主 chat 一条路:
      - REPL 主 chat 默认 USE_STREAM=1, 非流式路径在 minimax 上是冷代码.
        压缩用非流式之前实测会失败 — 既然主路径已经是流式, 压缩也跟着走.
      - 不需要工具调用 — 总结就是写一段文字, 我们禁用 tool_choice.

    失败原因打印到 stderr — 静默吞 LLM 错误的 debug 体验太差, 用户看到
    'mechanical fallback' 永远不知道为什么真 LLM 路径走不通.
    """
    prompt = _COMPRESSION_PROMPT.format(
        tail_n=tail_n, max_chars=COMPRESS_SUMMARY_MAX_CHARS, body=body,
    )
    messages = [{"role": "user", "content": prompt}]
    try:
        # 收集流式输出. 不打印 — 压缩本身已经有 _box 报告, 不要把摘要全文也喷屏.
        chunks: list[str] = []
        for delta in llm.chat_stream(messages, None):
            piece = delta.get("content") if isinstance(delta, dict) else None
            if isinstance(piece, str):
                chunks.append(piece)
        summary = "".join(chunks).strip()
    except Exception as e:
        print(f"{C.WARN}[compress] LLM call failed: "
              f"{type(e).__name__}: {e}{C.RESET}", file=sys.stderr)
        return None
    if not summary:
        print(f"{C.WARN}[compress] LLM returned empty content from chat_stream"
              f"{C.RESET}", file=sys.stderr)
        return None
    if len(summary) > COMPRESS_SUMMARY_MAX_CHARS * 2:
        # 模型不听话 — 截到 2x 上限. 摘要还能用, 只是没那么紧凑.
        summary = summary[:COMPRESS_SUMMARY_MAX_CHARS * 2]
    return summary


def _mechanical_summary(history: list[Message], split: int) -> str:
    """LLM 总结失败时的兜底: 用结构化字段拼一个粗糙摘要. 不调 LLM, 不会失败."""
    user_msgs = [m.content for m in history[1:split]
                 if m.role == "user" and not m.content.startswith("<reminder>")
                 and _COMPRESSION_MARKER not in m.content]
            ...
            for tc in m.tool_calls:
                ...
                if isinstance(args, str):
                    try:
                        ...
                        pass
    parts = ["(LLM summarization failed — mechanical summary follows)"]
    if user_msgs:
        first_user = user_msgs[0]
        if len(first_user) > 300:
            first_user = first_user[:300] + "..."
        parts.append(f"Original user request: {first_user}")
    if tool_calls_seen:
        # 计数前 N 个工具调用
        from collections import Counter
        counts = Counter(tool_calls_seen).most_common(8)
        parts.append("Tools used: " + ", ".join(f"{n}({c})" for n, c in counts))
    if files_touched:
        sample = sorted(files_touched)[:10]
        suffix = f" (+{len(files_touched) - 10} more)" if len(files_touched) > 10 else ""
        parts.append(f"Paths referenced: {', '.join(sample)}{suffix}")
    parts.append(f"Compressed {split - 1} messages from history.")
    return "\n".join(parts)
...
def _compress_history(
    ...
    keep_tail: int = COMPRESS_KEEP_TAIL,
) -> tuple[list[Message], dict] | None:
    """压缩 history, 返回 (新 history, 统计信息) 或 None (无需压).

    新 history 形态: [system, summary_user_msg, *tail_messages]
    summary_user_msg 用 <compressed>...</compressed> 标记 — 既给模型一个明确边界,
    也方便日后程序识别"这是个摘要 marker, 不是真用户消息".
    """
    if not history or history[0].role != "system":
        return None
    split = _find_compression_split(history, keep_tail)
    if split is None:
        return None

    body = _build_summary_body(history, split)
    summary_text: str | None = None
    used_llm = False
    if llm is not None:
        summary_text = _summarize_via_llm(llm, body, keep_tail)
        used_llm = summary_text is not None
    if summary_text is None:
        summary_text = _mechanical_summary(history, split)

    todo_block = ""
    if todo_items:
        # 把当前 todo (尤其 in_progress / pending) 显式塞进摘要 — 模型要继续干这些
        active = [t for t in todo_items if t.get("status") in ("in_progress", "pending")]
        if active:
            todo_lines = [f"  - [{t.get('status', '?')}] {t.get('text', '')}"
                          for t in active]
            todo_block = "\n\nOutstanding todos:\n" + "\n".join(todo_lines)

    summary_msg = Message.user(
        f"{_COMPRESSION_MARKER}\n{summary_text.strip()}{todo_block}\n{_COMPRESSION_MARKER_END}"
    )

    new_history = [history[0], summary_msg, *history[split:]]
    stats = {
        "before_count": len(history),
        "after_count": len(new_history),
        "summarized_count": split - 1,
        "kept_tail_count": len(history) - split,
        "used_llm": used_llm,
...
class ReActAgent:
    ...
    def _last_prompt_tokens(self) -> int:
        """最近一次 turn 的 prompt token 数. 没跑过 turn 时返回 0."""
    ...
    def _should_compress(self) -> bool:
        """判断是否到了该压缩的时机. 基于最近一次 turn 的真实 (或估算) prompt token."""
        if self.session.is_subagent:
            return False  # 子 agent 不压缩 — 短任务跑完就丢, 没必要
        threshold = int(CONTEXT_LIMIT * COMPRESS_AT)
        return self._last_prompt_tokens() >= threshold

    def compress_now(self, *, verbose: bool = True) -> dict | None:
        """显式压缩当前 history. 返回 stats 或 None (无需压).

        verbose=True 时打印一个 _box 报告. /compress 命令和自动触发都走这里.
        """
        sess = self.session
        result = _compress_history(
            sess.history,
            self.llm,
            todo_items=list(sess.todo.items),
        )
        if result is None:
            if verbose:
                _box("ℹ️  COMPRESS",
                     "history too short to compress — no-op", color=C.META)
            return None
        new_history, stats = result
        sess.history = new_history
        if verbose:
            mode = "LLM summary" if stats["used_llm"] else "mechanical fallback"
            body = (
                f"mode             : {mode}\n"
                f"messages before  : {stats['before_count']}\n"
                f"messages after   : {stats['after_count']}\n"
                f"summarized       : {stats['summarized_count']} messages\n"
                f"kept tail        : {stats['kept_tail_count']} messages\n"
                f"summary length   : {stats['summary_chars']} chars"
            )
            _box("🗜️  CONTEXT COMPRESSED", body, char="═", color=C.DONE)
        return stats

    def _maybe_compress_before_turn(self) -> None:
        """turn 开始前检查是否压缩. 注意: 第一个 turn 时 _last_usage 还没设, 不压."""
        if not hasattr(self, "_last_usage") or self._last_usage is None:
            return
        ...
        _box(
            "⚠️  CONTEXT THRESHOLD",
            f"prompt_tokens={self._last_prompt_tokens()} ≥ {threshold} "
            f"({int(COMPRESS_AT * 100)}% of {CONTEXT_LIMIT}). compressing...",
            color=C.WARN,
        )
        self.compress_now(verbose=True)
    ...
    def run(self, query: str, max_rounds: int = MAX_ROUNDS) -> None:
        ...
        for turn in range(1, max_rounds + 1):
            # v8: turn 开始前检查 context 压力. 第一轮 _last_usage 还没设, 内部会跳过.
            self._maybe_compress_before_turn()
...
可用斜杠命令:
  ...
  /todos             显示当前 todo 列表
  /history           显示消息条数统计
  /compress          手动压缩 history (中段对话 → LLM 摘要; 保头 + 保最后 10 条)
...
context 压缩 (v8): 当本 turn 的 prompt_tokens ≥ CONTEXT_LIMIT*COMPRESS_AT (默认
  70%) 时, 下一 turn 开始前自动压缩 history — 中段消息让 LLM 总结成一条 user
  消息, 保头 (system) + 保尾最后 10 条. tool_call/tool_result 配对永不被拆开.
  也可手动 /compress.
...
# ----- Session 持久化命令的 helpers (v7 新增) -----

# 文件名安全字符: 字母 / 数字 / - / _ . 拒绝其他 — 防路径穿越 (e.g. ../etc/passwd)
_SAFE_NAME_RE = re.compile(r"^[A-Za-z0-9_-]+$")


def _validate_session_name(name: str) -> str | None:
    """检查 session 名是否合法. 返回错误消息或 None (合法)."""
    if not name:
        return "session name cannot be empty"
    if len(name) > 64:
        return "session name too long (max 64 chars)"
    if not _SAFE_NAME_RE.match(name):
        return "session name must be [A-Za-z0-9_-]+ (got '%s')" % name
    return None


def _session_file(name: str) -> Path:
    """返回 session 名对应的文件路径. 不检查存在性."""
    return SESSIONS_DIR / f"{name}.json"


def _cmd_save(agent: ReActAgent, name: str) -> None:
    ...
    if target.exists():
        print(f"{C.WARN}overwriting existing session '{name}'{C.RESET}")
    try:
        target.write_text(json.dumps(agent.session.to_dict(), ensure_ascii=False, indent=2),
                          encoding="utf-8")
    except OSError as e:
        print(f"{C.ERR}save failed: {e}{C.RESET}")
        return
    n_msgs = len(agent.session.history)
    n_todos = len(agent.session.todo.items)
    p = agent.session.prompt_tokens_total
    c = agent.session.completion_tokens_total
    print(f"{C.OK}saved '{name}' ({n_msgs} messages, {n_todos} todos, "
          f"{_fmt_k(p)}+{_fmt_k(c)} tokens) → {target}{C.RESET}")


def _cmd_load(agent: ReActAgent, name: str) -> None:
    err = _validate_session_name(name)
    if err:
        print(f"{C.ERR}error: {err}{C.RESET}")
        return
    src = _session_file(name)
    if not src.is_file():
        print(f"{C.ERR}session '{name}' not found at {src}{C.RESET}")
        print(f"{C.DIM}use /sessions to list available{C.RESET}")
        return
    try:
        d = json.loads(src.read_text(encoding="utf-8"))
        loaded = Session.from_dict(d)
    except (OSError, json.JSONDecodeError, RuntimeError) as e:
        print(f"{C.ERR}load failed: {e}{C.RESET}")
        return
    # 替换 registry 上的 session — 跟 new_session() 同路径, 但用 load 出来的而不是新的
    agent.registry.session = loaded
    n_msgs = len(loaded.history)
    n_todos = len(loaded.todo.items)
    p = loaded.prompt_tokens_total
    c = loaded.completion_tokens_total
    print(f"{C.OK}loaded '{name}' ({n_msgs} messages, {n_todos} todos, "
          f"{_fmt_k(p)}+{_fmt_k(c)} tokens prior). read_cache cleared — "
          f"model must Read files again before writing.{C.RESET}")


def _cmd_sessions() -> None:
    """列出 SESSIONS_DIR 下所有 .json. 显示文件名 + 大小 + mtime 的相对时间."""
    if not SESSIONS_DIR.is_dir():
        print(f"{C.DIM}no sessions saved yet (dir would be {SESSIONS_DIR}){C.RESET}")
        return
    files = sorted(SESSIONS_DIR.glob("*.json"))
    if not files:
        print(f"{C.DIM}no sessions saved yet in {SESSIONS_DIR}{C.RESET}")
        return
    print(f"{C.DIM}sessions in {SESSIONS_DIR}:{C.RESET}")
    ...
    for f in files:
        try:
            ...
            age_str = _fmt_age(mtime_age)
            print(f"  {C.OK}{f.stem:24s}{C.RESET}  "
                  f"{st.st_size:>6d} bytes  "
                  f"{C.DIM}({age_str} ago){C.RESET}")
        except OSError:
            pass


def _cmd_del(name: str) -> None:
    err = _validate_session_name(name)
    if err:
        print(f"{C.ERR}error: {err}{C.RESET}")
        return
    ...
    if not target.is_file():
        print(f"{C.ERR}session '{name}' not found{C.RESET}")
        return
    try:
        target.unlink()
        print(f"{C.OK}deleted session '{name}'{C.RESET}")
    except OSError as e:
...
def _print_banner(agent: ReActAgent, terminal: TerminalTool) -> None:
    ...
    print(f"  {C.DIM}context:{C.RESET} limit {_fmt_k(CONTEXT_LIMIT)} tokens, "
          f"auto-compress at {int(COMPRESS_AT * 100)}% (keep tail {COMPRESS_KEEP_TAIL})")
...
def repl() -> None:
    ...
    while True:
        ...
        if line.startswith("/"):
            # 拆成 cmd + args. /save NAME → cmd="save", args="NAME"
            ...
            # ----- v8: 手动压缩 -----
            if cmd == "compress":
                agent.compress_now(verbose=True)
                continue
            # ----- v7: session 持久化命令 -----
            if cmd == "save":
                _cmd_save(agent, args)
                continue
            if cmd == "load":
                _cmd_load(agent, args)
                continue
            if cmd == "sessions":
                _cmd_sessions()
                continue
